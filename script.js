document.addEventListener('DOMContentLoaded', () => {
    ...
    if(minesPlayBtn) {
        ...
        function initGridUI() {
            ...
            for(let i=0; i<25; i++) {
                ...
                tile.dataset.i = i;
                tile.innerHTML = '<span class="tile-mark">RoBet</span>';
        ...
        window._minesRestore = function(session) {
            ...
            tiles.forEach((t, i) => {
                ...
                if (revSet.has(i)) {
                    ...
                    t.innerHTML = '<span class="tile-mark">RoBet</span>';
                } else {
                    t.innerHTML = '<span class="tile-mark">RoBet</span>';
                }
            });
            ...
            for(let i=0; i<25; i++) {
                if(!tiles[i].classList.contains('revealed')) {
                    ...
                    if(mGrid[i]) {
                        ...
                        tiles[i].innerHTML = '<span class="tile-mark" style="opacity:0.4">RoBet</span>';
                    }
                }
            }
    ...
    if (aviaPlayBtn && aviaCanvas && aviaStage) {
        ...
        /**
         * Loose, floaty flight — drifts toward the next area with wander (not a visible “scripted” arc).
         * On “pickup” moment: prize/boost nudges up, rocket nudges down; final land/crash is calmer and tighter.
         */
        function aviaAnimateSegment(from, to, baseMs, ev, segIndex) {
            return new Promise((resolve) => {
                ...
                const pathAng = Math.atan2(dy, dx);
                const perpAng = pathAng + Math.PI / 2;
                const seed = segIndex * 2.413 + fx * 0.017 + fy * 0.011;
                const isFinal = ev && (ev.type === 'land' || ev.type === 'crash');
                let durationMs = baseMs * (0.88 + dist / 165);
                durationMs = Math.min(3600, Math.max(baseMs * 0.58, durationMs));
                if (isFinal) durationMs *= 0.92;

                const hitU = 0.74 + (Math.sin(seed) * 0.5 + 0.5) * 0.12;
                let hitApplied = false;
                let bumpWy = 0;
                const t0 = performance.now();

                function sampleDrift(uLin) {
                    const p = aviaEaseInOutCubic(uLin);
                    const slack = isFinal ? 0.92 : 0.78;
                    const bx = fx + dx * Math.min(1, p * slack + (1 - slack) * p * p * 0.35);
                    const by = fy + dy * Math.min(1, p * slack + (1 - slack) * p * p * 0.35);

                    const wMagA = isFinal ? 10 : 36 + Math.sin(seed * 3) * 10;
                    const wMagB = isFinal ? 5 : 22;
                    const w1 = wMagA * Math.sin(p * Math.PI * 4.4 + seed);
                    const w2 = wMagB * Math.sin(p * Math.PI * 7.1 + seed * 1.9);
                    const w3 = isFinal ? 0 : 16 * Math.sin(uLin * Math.PI * 2.8 + seed * 2.2);
                    let ox = Math.cos(perpAng) * (w1 + w2 * 0.55);
                    let oy = Math.sin(perpAng) * (w1 + w2 * 0.55);

                    const fadeWig = isFinal ? 1 - p * 0.85 : 1 - p * 0.25;
                    ox *= fadeWig;
                    oy *= fadeWig;

                    const bobA = isFinal ? 5 : 15;
                    const bobB = isFinal ? 0 : 11;
                    const bob =
                        bobA * Math.sin(uLin * Math.PI * 2.1 + seed * 3) +
                        bobB * Math.sin(uLin * Math.PI * 4.6 + seed);

                    return { wx: bx + ox, wy: by + oy + bob };
                }
                ...
                function tick(now) {
                    ...
                    if (!hitApplied && rawU >= hitU && ev && !isFinal) {
                        hitApplied = true;
                        if (ev.type === 'prize') bumpWy = -26 - Math.min(10, (ev.add || 1) * 1.2);
                        else if (ev.type === 'boost') bumpWy = -20;
                        else if (ev.type === 'rocket') bumpWy = 24;
                        else if (ev.type === 'cruise') bumpWy = (Math.sin(seed * 5) * 5);
                    }

                    let pos = sampleDrift(rawU);
                    pos.wy += bumpWy;
                    bumpWy *= 0.9;
                    ...
                    const du = 0.018;
                    const pA = sampleDrift(Math.max(0, rawU - du));
                    const pB = sampleDrift(Math.min(1, rawU + du));
                    let ax = pB.wx - pA.wx;
                    let ay = pB.wy - pA.wy + bumpWy * 0.08;
                    if (ax * ax + ay * ay < 4) {
                        ax = Math.cos(pathAng) * 3;
                        ay = Math.sin(pathAng) * 3;
                    }
                    aviaPlane.angle = Math.atan2(ay, ax);
        ...
        aviaPlayBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                for (let k = 0; k < events.length; k++) {
                    ...
                    await aviaAnimateSegment(from, to, ms, ev, k);
                    aviaIncoming = null;
...
let _roBalance = 0.00;    // RoBet (main currency)
...
function renderLiveFeedRows(events) {
    ...
    slice.forEach((ev) => {
        ...
        tr.innerHTML =
            ...
            escapeFeedHtml(user) +
            '</div></td>' +
            '<td><span class="currency-inline">RoBet</span> ' +
            bet.toFixed(2) +
            '</td>' +
            ...
            '<td class="' +
            payoutClass +
            '"><span class="currency-inline">RoBet</span> ' +
            payoutStr +
            '</td>' +
...
function patchBlackjackBalance() {
    ...
    playBtn.addEventListener('click', function(e) {
        ...
        if(playBtn.textContent.trim() === 'Place bet') {
            ...
            if(bet > roBalance) {
                e.stopImmediatePropagation();
                if(bjMsg) { bjMsg.textContent = 'Not enough RoBet!'; bjMsg.style.color='var(--red)'; bjMsg.style.display='block'; }
                return;
            }
...
function patchMinesBalance() {
    ...
    playBtn.addEventListener('click', function(e) {
        if(playBtn.textContent.trim() === 'Start new game') {
            ...
            if(bet <= 0 || bet > roBalance) {
                e.stopImmediatePropagation();
                if(minesMsg) { minesMsg.textContent= bet <= 0 ? 'Enter a valid bet amount!' : 'Not enough RoBet!'; minesMsg.style.color='var(--red)'; minesMsg.style.display='block'; }
                return;
            }
            // Do NOT deductBet — server handles it atomically
...
function patchTowersBalance() {
    ...
    playBtn.addEventListener('click', function(e) {
        if(playBtn.textContent.trim() === 'Start new game') {
            ...
            if(bet <= 0 || bet > roBalance) {
                e.stopImmediatePropagation();
                if(tMsg) { tMsg.textContent= bet <= 0 ? 'Enter a valid bet amount!' : 'Not enough RoBet!'; tMsg.style.color='var(--red)'; tMsg.style.display='block'; }
                return;
            }
            // Do NOT deductBet — server handles it atomically
...
async function confirmWithdraw() {
    ...
    if(coins > roBalance) {
        showErr('Not enough RoBet balance to withdraw this amount.');
        return;
    }
...
function confirmBuyXp() {
    ...
    if(amt > roBalance) {
        btn.textContent = 'Not enough RoBet';
        btn.style.background = 'var(--red)';
        setTimeout(() => { btn.textContent = old; btn.style.background = ''; }, 2000);
        return;
    }
        ...
        if (robloxUserId && data.recipientId === robloxUserId) {
            ...
            const tipDisp =
                typeof data.amount === 'number' && Number.isFinite(data.amount)
                    ...
                    : data.amount;
            toast.textContent = `🎉 You were tipped ${tipDisp} RoBet from ${data.sender}!`;
...
function renderTournamentBannerStrip(tournaments) {
    ...
    strip.innerHTML = tournaments
        .map((t) => {
            ...
            const cur = t.prizeCurrency === 'zh' ? 'ZH$' : 'RoBet';
...
function cbRenderCasesGrid() {
    ...
    grid.innerHTML = _cbCases.map(c => `
        <div class="cb-case-card" style="--case-color:${c.color}" onclick="cbShowConfirmCaseModal('${c.id}')">
            ...
            <div class="cb-case-price">${c.price.toLocaleString()} <span>RoBet</span></div>
    ...
    el.innerHTML = _cbBattles.map(b => {
        ...
        return `
            <div class="cb-battle-row" onclick="cbOpenBattleRoom('${b.id}')">
                ...
                <div class="cb-battle-row-price">${(b.casePrice*b.rounds).toLocaleString()} RoBet</div>
            </div>
        `;
    }).join('');
}
...
function cbRenderPreviewReel() {
    ...
    reel.innerHTML = sorted.map(item => `
        <div class="cb-preview-item rarity-${item.rarity}">
            ...
            <div class="pi-val" style="color:${RARITY_COLORS[item.rarity]||'#fff'}">${item.value?item.value.toLocaleString()+' RoBet':'—'}</div>
            <div class="pi-tooltip">${item.name}${item.value?' · '+item.value.toLocaleString()+' RoBet':''}</div>
        </div>
    `).join('');
...
function cbUpdateCreateCost() {
    ...
    if (el) el.textContent = cost.toLocaleString() + ' RoBet (your entry)';
}
...
async function cbOpenCaseModal(caseId) {
    ...
    track.innerHTML = items.map((item, i) => `
        <div class="cb-horiz-item cb-item-rarity-${item.rarity}" data-idx="${i}">
            ...
            <div class="cb-horiz-item-val">${item.value?item.value.toLocaleString()+' RoBet':'—'}</div>
        </div>
    `).join('');
...
function cbRenderBattleRoom(b) {
            ...
            <div class="team-side left">
                ...
                <div class="cb-1v1-player">
                    ...
                    <div class="details">
                        ...
                        <div class="total" id="btotal-${p.userId}">${p.total.toLocaleString()} RoBet</div>
                    </div>
                </div>`).join('')}
    ...
    if (b.status === 'done' && (b.winner || b.isTie)) {
        ...
        if (b.isTie) {
            ...
            winnerEl.innerHTML = `<h3>🏆 ${b.winner.username} Won! ${payoutAmount.toLocaleString()} RoBet</h3>`;
        }
    } else {
...
function cbRollCardHTML(item) {
    ...
    return `
        <div class="cb-horiz-item cb-item-rarity-${item.rarity}" style="animation: cbRollIn .3s cubic-bezier(.34,1.56,.64,1); width:110px; min-height:72px; margin-right:4px;">
            ...
            <div class="cb-horiz-item-val" style="font-size:10px;">${item.value?item.value.toLocaleString()+' RoBet':'—'}</div>
        </div>
    `;
}
            ...
            try {
                ...
                const spinPromises = results.map(async (r) => {
                    ...
                    return cbAnimateSpinDirection(track, targetVal, 3800, 'X').then(async () => {
                        ...
                        if (totalEl) totalEl.textContent = r.total.toLocaleString() + ' RoBet';
                    });
                });
